...
impl EmploymentInsuranceRatesAndAmounts {
    ...
    pub fn init(version: &Version) -> Result<EmploymentInsuranceRatesAndAmounts, Box<dyn StdError>> {
        ...
        if let Some(QC) = records.iter().position(|rec| rec.ei == "QC") {
            ...
            if let Some (CA) = records.iter().position(|rec| rec.ei == "Canada except QC") {
                return Ok(
                    EmploymentInsuranceRatesAndAmounts {
...
pub struct EI_RA {
    #[serde(rename="EI")]
    ei: String,
    #[serde(deserialize_with="quoted_f64", rename="Maximum Annual Insurable Earnings")]
    pub MaxAIE: f64,
    ...
    pub ER_CR: f64,
    #[serde(deserialize_with="quoted_f64", rename="Maximum Annual Employee Premium")]
    pub MaxAEEP: f64,
    #[serde(deserialize_with="quoted_f64", rename="Maximum Annual Employer Premium")]
    pub MaxAERP: f64,
}
